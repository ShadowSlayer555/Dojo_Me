...
export function Auth({ onLogin }: { onLogin: (user: any) => void }) {
    ...
    try {
      if (requireVerification) {
        ...
        if (res.requireVerification) {
          ...
          setSuccessMsg('A 6-digit verification code has been sent to your email. Please check your junk/spam folder if you don\'t see it.');
        }
      }
    } catch (err: any) {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setSuccessMsg('A new code has been sent to your email. Please check your junk/spam folder.');
    } catch (err: any) {
      setError(err.message);
    }
  };
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-md p-8 text-center">
          ...
          <p className="text-slate-600 mb-6">Enter the 6-digit code sent to <strong>{email}</strong>. <br/><span className="text-sm text-red-500">Please check your junk/spam folder if you don't see it.</span></p>
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-md p-8">
        <h2 className="text-2xl font-bold text-center text-slate-900 mb-6">
          {isLogin ? 'Sign In to Dojo Me' : 'Create an Account'}
        </h2>
